...
AND     A.rule_id IN (1912, 1916, 1914, 223217, 222676)
AND     A.ticket_create_time>='${DATE-59}'
AND     A.ticket_create_time<'${DATE+1}'
AND     A.ticket_type_code=3
AND     A.is_deleted=0
"""
shop_lz_seed_penalize=sql(shop_lz_seed_penalize_sql).toPandas()
print(len(shop_lz_seed_penalize))
print(shop_lz_seed_penalize[:3])

# %% [code]
# 1912 recall_product_verify_result -> verified_order_info -> product_id / violation_detail:commodity_id 驳回原因不为空的商品ID,部分数据无法解析
#      recall_product_verify_result -> verified_order_info -> product_id 有可能多个商品ID-拼接,解析成商品列表,即一个罚单对应多个商品
# 1916 recall_product_verify_result -> verified_order_info -> product_id / violation_detail:commodity_id 驳回原因不为空的商品ID,部分数据无法解析
# 1914 recall_product_verify_result -> verified_order_info -> product_id / violation_detail:commodity_id 驳回原因不为空的商品ID,部分数据无法解析
# 223217 无
# 222676 无

# %% [code]
output = []
for idx in tqdm(range(len(shop_lz_seed_penalize))):
    shop_id = shop_lz_seed_penalize['shop_id'].iloc[idx]
    rule_id = shop_lz_seed_penalize['rule_id'].iloc[idx]
    penalize_id = shop_lz_seed_penalize['penalize_id'].iloc[idx]
    penalize_time = shop_lz_seed_penalize['penalize_time'].iloc[idx]
    final_factor_info_list_map = shop_lz_seed_penalize['final_factor_info_list_map'].iloc[idx]
...
# %% [code]
dataframe2hive(df=output_df, table_name='ecom_govern_community.groupaudit_shop_seed_penalize_product_info', date=today, risk_type='lz', spark=spark)
