...
-- CreateTime: 2025-08-25 21:34:25
-- Description:
-- Update: Task Update Description
-- ********************************************************************
INSERT OVERWRITE TABLE ecom_govern_community.groupaudit_shop_recallstrategy_v1_filter_groupshopinfo PARTITION (date = '${date}', risk_type = 'jh')
SELECT  group_type,
        group_id,
        group_size,
        shop_id,
        ...
        stop_status_code,
        shop_open_time,
        is_jh_seed AS is_seed,
        jh_seed_penalize_info AS seed_penalize_info,
        jh_first_seed_penalize_time AS first_seed_penalize_time,
        product_cnt,
...
FROM    (
            SELECT  A.group_type,
                    ...
                    A.shop_open_time,
                    A.is_jh_seed,
                    A.jh_seed_penalize_info,
                    A.jh_first_seed_penalize_time,
                    A.product_cnt,
                    ...
                    count(CASE WHEN A.is_jh_seed = 1 THEN A.shop_id ELSE NULL END) OVER (
                        PARTITION BY
                                A.group_type,
                                A.group_id
                    ) AS group_jh_seed_cnt,
                    count(
                        CASE WHEN A.is_jh_seed = 1 OR A.p60d_update_product_cnt >= 1 THEN A.shop_id
                             ELSE NULL
                        END
                    ) OVER (
                        PARTITION BY
                    ...
                    count(
                        CASE WHEN A.is_jh_seed = 1 AND B.first_seed_grouprejectpenalize_time IS NULL THEN A.shop_id
                             ELSE NULL
                        END
                    ) OVER (
                        PARTITION BY
                                A.group_type,
                                A.group_id
                    ) AS group_worth_reejctandpenalize_jh_seed_cnt
