...
for group_id in tqdm(group_id_list):
    ...
    # 聚集性过滤
    if len(seed_shop_list) + len(unseed_shop_list) <= 2:
        continue
    # 进审size过大切分
    if len(seed_shop_list) + len(unseed_shop_list) <= 10:
    ...
    elif len(seed_shop_list) <= 3:
        gap_cnt = 10 - len(seed_shop_list)
        bucket_cnt = math.ceil(len(unseed_shop_list) / gap_cnt)
        sample_cnt = math.ceil(len(unseed_shop_list) / bucket_cnt)
        random.shuffle(unseed_shop_list)
        split_list = split_chunks(sample_cnt, unseed_shop_list)
        sub_group_id = 10000
        for temp_split in split_list:
            group_shop_list = seed_shop_list + temp_split
            if len(list(set(group_shop_list) & set(worthpenalize_seed_shop_list))) == 0:
                continue
        ...
        seed_end_sign = 0
        unseed_end_sign = 0
        while seed_end_sign == 0 or unseed_end_sign == 0:
...
# %% [code]
rule_label_dict = {
    216941:	['底线问题-假货/盗版',	'承认出售假货-商家'],
    216949:	['底线问题-假货/盗版',	'承认出售假货-商家'],
    208647:	['底线问题-假货/盗版',	'承认出售假货-商家'],
    208648:	['底线问题-假货/盗版',	'承认出售假货-商家'],
    219466:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    219472:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    224000:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    208722:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    208717:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    208715:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    208720:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    208716:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    219467:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    208721:	['底线问题-假货/盗版',	'品牌方/监管鉴定假货-商家'],
    220713:	['底线问题-假货/盗版',	'三方机构鉴定假货-商家'],
    220230:	['底线问题-假货/盗版',	'三方机构鉴定假货-商家'],
    222765:	['底线问题-假货/盗版',	'三方机构鉴定假货-商家'],
    219490:	['底线问题-假货/盗版',	'三方机构鉴定假货-商家'],
    219489:	['底线问题-假货/盗版',	'三方机构鉴定假货-商家'],
    221454:	['底线问题-假货/盗版',	'三方机构鉴定假货-商家'],
    220086:	['底线问题-假货/盗版',	'三方机构鉴定假货-商家'],
    220715:	['底线问题-假货/盗版',	'三方机构鉴定假货-商家'],
    220714:	['底线问题-假货/盗版',	'三方机构鉴定假货-商家'],
    208651:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    208696:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    208718:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    212450:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    219468:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    219469:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    219470:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    219471:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    208649:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    208650:	['底线问题-假货/盗版',	'实物鉴定为假(商家)'],
    208719:	['底线问题-假货/盗版',	'实物鉴定为假(商家)']
}
...
for group_info in tqdm(group_output):
    ...
    recall_reason = '假货种子扩散'
    seed_shop_group_type = '假货'
    ...
    for shop_id in group_info['group_shop_list']:
        ...
        if shop_info['seed_penalize_info'] is not None:
            for seed_penalize_info in shop_info['seed_penalize_info']:
                ...
                formatted_penalize_reason = penalize_reason.replace('）', ')').replace('（', '(').replace('：',':').replace(' ', '')
                brand_pattern = re.compile(r'\(涉及品牌:(.*?)\)', re.DOTALL)
                penalize_brand = re.findall(brand_pattern, formatted_penalize_reason)[0]
...
# %% [code]
dataframe2hive(df=group_result_df, table_name='ecom_govern_community.groupaudit_shop_recallstrategy_v1_result_groupshopinfo', date=today, risk_type='jh', spark=spark)
